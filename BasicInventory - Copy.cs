...
namespace BasicInventory
{
    public class Program : MyGridProgram
    {
        ...
        public ScriptProgram Initialize()
        {
            ...
            var helloLcd = new NamedBlockProgram<WriteTextOnLcd>(
    ...
    public class TextToLcdBuffer : BufferedProgram<string, WriteTextOnLcd, NamedBlockProgram<WriteTextOnLcd>>
    {
        public TextToLcdBuffer(NamedBlockProgram<WriteTextOnLcd> wrapped, string name) : base(wrapped, name)
        {
        }
    }
    ...
    public class GroupProgram<TBlockAction> : BlockProgram<TBlockAction> where TBlockAction : BlockAction
    {
...
, BlockAction.Execute);
    ...
    public class NamedBlockProgram<TBlockAction> : BlockProgram<TBlockAction> where TBlockAction : BlockAction
    {
    ...
    public class BufferedProgram<TInput, TBlockAction, TProgram> : ScriptProgram, WithInput<TInput>
        where TProgram: BlockProgram<TBlockAction> 
        where TBlockAction: BlockAction, WithInput<TInput>
    {
        protected TProgram Wrapped { get; private set; }

        public BufferedProgram(TProgram wrapped, string name) : base(wrapped.Env, name)
        {
            Inputs = new Stack<object>();
        }

        private Stack<object> Inputs;
        ...
        protected override void OnMain(string param = "")
        {
            ...
            Wrapped.BlockAction.Input = (TInput)(Inputs.Pop());
            Wrapped.Main(param);
        }
    }

    public class BlockProgram<TBlockAction> : ScriptProgram where TBlockAction: BlockAction
    {
        public readonly TBlockAction BlockAction;
        ...
        {
            BlockAction = (TBlockAction)blockAction;
...
, BlockAction.Execute);
    ...
    public class MainScriptProgram : ScriptProgram
    {
        ...
        public override string ToString()
        {
            ...
            ret += ",\n childs:{";
            Ext.ForEach(ScriptActions, action => { ret += action.ToString() + ",\n"; });
            ret += "}}\n";
    ...
    public class WriteTextOnLcd : BlockAction, WithInput<string>
    {
        public WriteTextOnLcd() : base("WriteTextOnLcd") { }
        public string Input { get; set; }        
        public override void Execute(IMyTerminalBlock block)
        {
            var tp = block as IMyTextPanel;
            if (tp == null || Input == null) return;
            tp.WritePublicText(Input);
    ...
    public interface WithInput<T> { T Input { set; } }
    public delegate void OutputChanged<T>(T value);
    public interface WithOutput<T> { event OutputChanged<T> OnOutput; }
    #endregion
    #endregion
